...
import { calculateCustomEnergyMaximum, CUSTOM_ENERGY_ICONS, formatEnergyCost, normalizeCustomEnergy, type CustomEnergy, type CustomEnergyIcon } from "@shared/customEnergies";
...
type InventoryItem = { id: string; name: string; category: string; quantity: number; weight: number; carried: boolean; equipped: boolean; hand?: string; modifier?: string; description?: string };
...
type Power = { id: string; name: string; source: string; type: "Ofensivo" | "Defensivo" | "Utilidade" | "Controle"; level: number; bonus?: number; skillId?: string; fpCost: number; resourceId?: string; resourceCost?: number; pointCost: number; range: string; duration?: string; area?: string; resistance?: string; prerequisites?: string; notes?: string; damage: string; effect: string; combatReady: boolean };
...
type AllySecondary = { willBonus?: number; perBonus?: number; speedBonus?: number; moveBase?: number; moveBonus?: number; dodgeBonus?: number };
type Ally = { id: string; name: string; relation: string; description: string; points: number; cost: number; hpCurrent: number; hpMax: number; status: string; portraitUrl?: string | null; type?: string; race?: string; appearance?: string; personality?: string; history?: string; motivation?: string; notes?: string; powerPercent?: 25 | 50 | 75 | 100 | 150; frequency?: 6 | 9 | 12 | 15; isDependent?: boolean; attributes?: { st: number; dx: number; iq: number; ht: number }; secondary?: AllySecondary; fpCurrent?: number; fpMax?: number; advantages?: Trait[]; disadvantages?: Trait[]; skills?: Skill[]; attacks?: Attack[]; inventory?: InventoryItem[]; conditions?: string[] };
...
const secondaryModifierFields: Array<{ field: keyof typeof SECONDARY_POINT_COSTS; label: string }> = [
  { field: "hpBonus", label: "Bônus de PV" }, { field: "fpBonus", label: "Bônus de PF" }, { field: "willBonus", label: "Vontade" },
  { field: "perBonus", label: "Percepção" }, { field: "speedBonus", label: "Velocidade" }, { field: "moveBonus", label: "Movimento" }, { field: "dodgeBonus", label: "Esquiva" },
];
...
const MISSION_DIFFICULTIES = ["Baixa", "Média", "Alta", "Épica", "Lendária"] as const;
const getMissionDifficultyLabel = (mission: Mission) => mission.difficulty === "Personalizada" ? mission.customDifficulty?.trim() || "Personalizada" : mission.difficulty;
...
const normalizeAlly = (ally: Ally) => ({
  ...
  attributes: ally.attributes || { st: 10, dx: 10, iq: 10, ht: 10 }, secondary: { willBonus: 0, perBonus: 0, speedBonus: 0, moveBase: 5, moveBonus: 0, dodgeBonus: 0, ...ally.secondary }, fpCurrent: ally.fpCurrent ?? 10, fpMax: ally.fpMax ?? 10,
...
const initialSheet: Sheet = {
  ...
  secondary: { hpCurrent: 12, hpBonus: 0, fpCurrent: 11, fpBonus: 0, willBonus: 1, perBonus: 1, speedBonus: 0, moveBase: 5, moveBonus: 0, dodgeBonus: 0, thrustBonus: 0, swingBonus: 0 },
  ...
  inventory: [
    ...
    { id: "item-2", name: "Mochila de campanha", category: "Ferramentas", quantity: 1, weight: 1.5, carried: true, equipped: true, hand: "Mochila", description: "Mochila de campanha para levar itens e suprimentos." },
...
export default function Home() {
  ...
  const calculated = useMemo(() => {
    ...
    const basicLift = calculateBasicLiftKg(sheet.attributes.st);
    const baseStrengthDamage = calculateStrengthDamage(sheet.attributes.st);
    const strengthDamage = {
      ...baseStrengthDamage,
      thrust: applyDamageBonus(baseStrengthDamage.thrust, sheet.secondary.thrustBonus || 0),
      swing: applyDamageBonus(baseStrengthDamage.swing, sheet.secondary.swingBonus || 0),
    };
    ...
    return { hpMax, fpMax, will, perception, speed, basicLift, strengthDamage, carriedWeight, encumbrance, encName: encNames[encumbrance], dodge, move, attributePoints, secondaryPoints, advantagePoints, disadvantagePoints, skillPoints, powerPoints, allyCost, totalSpent, available };
  }, [sheet]);
  const customEnergies = useMemo(() => (sheet.customEnergies || []).map(normalizeCustomEnergy), [sheet.customEnergies]);
  ...
  const updateCustomEnergy = (id: string, field: keyof CustomEnergy, value: string | number) => {
    setSheet((current) => ({ ...current, customEnergies: (current.customEnergies || []).map((energy) => {
      if (energy.id !== id) return normalizeCustomEnergy(energy);
      const candidate = { ...normalizeCustomEnergy(energy), [field]: field === "maximum" || field === "current" || field === "bonus" ? number(String(value)) : value } as CustomEnergy;
      return normalizeCustomEnergy(candidate);
    }) }));
  };

  const setCustomEnergyResource = (id: string, value: number, record = true) => {
    const previousEnergy = customEnergies.find((energy) => energy.id === id);
    if (!previousEnergy) return;
    const next = clampResource(value, calculateCustomEnergyMaximum(previousEnergy));
    setSheet((current) => ({ ...current, customEnergies: (current.customEnergies || []).map((energy) => energy.id === id ? normalizeCustomEnergy({ ...energy, current: next }) : normalizeCustomEnergy(energy)) }));
    if (record && next !== previousEnergy.current) addLog(describeResourceChange(previousEnergy.name, previousEnergy.current, next), "health");
  };

  const removeCustomEnergy = (id: string, name: string) => {
    setSheet((current) => ({ ...current, customEnergies: (current.customEnergies || []).filter((energy) => energy.id !== id), powers: (current.powers || []).map((power) => power.resourceId === id ? { ...power, resourceId: undefined, resourceCost: undefined } : power) }));
    addLog(`Removeu a energia ${name || "sem nome"} e devolveu seus poderes ao custo em PF.`, "note");
  };

  const getPowerResource = (power: Power) => {
    const energy = power.resourceId ? customEnergies.find((item) => item.id === power.resourceId) : undefined;
    const cost = Math.max(0, Number(energy ? power.resourceCost ?? power.fpCost : power.fpCost));
    return energy ? { id: energy.id, name: energy.name, current: energy.current, maximum: calculateCustomEnergyMaximum(energy), cost } : { id: "pf", name: "PF", current: sheet.secondary.fpCurrent, maximum: calculated.fpMax, cost };
  };
  ...
  const addBackpack = () => {
    if (sheet.inventory.some((item) => item.name.trim().toLocaleLowerCase().includes("mochila"))) {
      addLog("A ficha já possui uma mochila no inventário.", "note");
      return;
    }
    setSheet((current) => ({ ...current, inventory: [...current.inventory, { id: makeId(), name: "Mochila de campanha", category: "Ferramentas", quantity: 1, weight: 1.5, carried: true, equipped: true, hand: "Mochila", modifier: "", description: "Mochila de campanha para levar itens e suprimentos." }] }));
    addLog("Adicionou uma mochila de campanha ao equipamento.", "note");
  };
  ...
  const usePower = (power: Power) => {
    ...
    const resource = getPowerResource(power);
    if (resource.current < resource.cost) { addLog(`Não há ${resource.name} suficiente para ativar ${power.name}.`, "note"); return; }
    if (resource.id === "pf") setSheet((current) => ({ ...current, secondary: { ...current.secondary, fpCurrent: current.secondary.fpCurrent - resource.cost } }));
    else setCustomEnergyResource(resource.id, resource.current - resource.cost, false);
    addLog(`Ativou ${power.name} e gastou ${formatEnergyCost(resource.name, resource.cost)}.`, "health");
  ...
  const addHomebrewToSheet = (rawEntry: HomebrewEntry) => {
    ...
    setSheet((current) => {
      ...
      if (entry.category === "Energia") {
        if ((current.customEnergies || []).some((energy) => energy.homebrewId === entry.id)) return current;
        const maximum = Math.max(1, detailNumber("maximum", 10));
        return { ...current, customEnergies: [...(current.customEnergies || []), normalizeCustomEnergy({ id: makeId(), name: entry.title || "Nova energia", maximum, current: detailNumber("current", maximum), icon: String(details.icon || "Arcana") as CustomEnergyIcon, description, source: entry.source, homebrewId: entry.id })] };
      }
  ...
  const removeHomebrew = (id: string, title: string) => {
    setSheet((current) => {
      ...
      const removedEnergyIds = (withoutRace.customEnergies || []).filter((energy) => energy.homebrewId === id).map((energy) => energy.id);
      return {
        ...withoutRace,
        homebrew: (withoutRace.homebrew || []).filter((entry) => entry.id !== id),
        customEnergies: (withoutRace.customEnergies || []).filter((energy) => energy.homebrewId !== id),
        powers: (withoutRace.powers || []).map((power) => removedEnergyIds.includes(power.resourceId || "") ? { ...power, resourceId: undefined, resourceCost: undefined } : power),
      };
    });
  ...
  const updateAllySecondary = (id: string, field: keyof AllySecondary, value: number) => {
    setSheet((current) => ({ ...current, allies: (current.allies || []).map((ally) => {
      if (ally.id !== id) return ally;
      const full = normalizeAlly(ally);
      return { ...full, secondary: { ...full.secondary, [field]: value } };
    }) }));
  };
  ...
  const addAlly = () => {
    const ally: Ally = { id: makeId(), name: "Novo aliado", relation: "Aliado", description: "Descreva como este aliado ajuda na campanha.", points: 25, cost: 1, hpCurrent: 10, hpMax: 10, fpCurrent: 10, fpMax: 10, status: "Pronto", type: "Individual", race: "Humano", powerPercent: 25, frequency: 12, attributes: { st: 10, dx: 10, iq: 10, ht: 10 }, secondary: { willBonus: 0, perBonus: 0, speedBonus: 0, moveBase: 5, moveBonus: 0, dodgeBonus: 0 }, advantages: [], disadvantages: [], skills: [], attacks: [], inventory: [], conditions: [] };
  ...
  const exportJson = () => {
    const exportedSheet = {
      ...
      calculations: {
        ...
        thrustDamage: calculated.strengthDamage.thrust,
        swingDamage: calculated.strengthDamage.swing,
        customEnergies,
      ...
      <main className="codex-main">
        ...
        <section className="banner" style={{ backgroundImage: `linear-gradient(90deg, rgba(248, 244, 235, .96) 0%, rgba(248, 244, 235, .82) 53%, rgba(248, 244, 235, .22) 100%), url(${BANNER})` }}>
          ...
          <div className="banner__instruments">
            ...
            <div><HeartPulse size={14} /><span>PV</span><b>{sheet.secondary.hpCurrent}/{calculated.hpMax}</b></div>
            <div><Activity size={14} /><span>PF</span><b>{sheet.secondary.fpCurrent}/{calculated.fpMax}</b></div>
            <div><Shield size={14} /><span>Esquiva</span><b>{calculated.dodge}</b></div>
            <div><Crosshair size={14} /><span>Movimento</span><b>{calculated.move}</b></div>
        ...
        <div className="codex-content">
          <section id="visao-geral" className={`codex-section ${activeSection === "visao-geral" ? "is-active" : "is-hidden"}`}>
            ...
            <div className="overview-grid">
              ...
              <div className="attribute-panel">
                ...
                <div className="derived-list">
                  <div><span>ST para dano</span><b>Golpe {calculated.strengthDamage.thrust} · Balanço {calculated.strengthDamage.swing}</b><small>Calculado automaticamente por ST {calculated.strengthDamage.st}</small></div>
                  <div><span>Carga básica</span><b>{format(calculated.basicLift, 1)} {UNIVERSAL_WEIGHT_UNIT}</b><small>Referência convertida da ST para carga em quilogramas</small></div>
                </div>
              </div>
            </div>

            <div className="metrics-grid">
              <MetricCard label="PV" value={`${sheet.secondary.hpCurrent}/${calculated.hpMax}`} detail="Pontos de vida" tone="red" />
              <MetricCard label="PF" value={`${sheet.secondary.fpCurrent}/${calculated.fpMax}`} detail="Fadiga" tone="moss" />
              <MetricCard label="Vontade" value={calculated.will} detail={`IQ ${sheet.attributes.iq} + ${sheet.secondary.willBonus}`} />
              <MetricCard label="Percepção" value={calculated.perception} detail={`IQ ${sheet.attributes.iq} + ${sheet.secondary.perBonus}`} />
              <MetricCard label="Velocidade" value={format(calculated.speed, 2)} detail="Velocidade básica" />
              <MetricCard label="Movimento" value={calculated.move} detail={`Carga ${calculated.encName.toLowerCase()}`} />
              <MetricCard label="Esquiva" value={calculated.dodge} detail="Defesa ativa" />
            </div>
            ...
            <div className="resource-grid">
              <div className="paper-card resource-card">
                ...
                <div className="custom-energy-session"><div className="custom-energy-session__head"><span className="eyebrow">ENERGIAS HOMEBREW</span><button type="button" onClick={() => navigateTo("homebrew")}><Plus size={13} /> Criar na Homebrew</button></div>{customEnergies.length ? <div className="custom-energy-session__list">{customEnergies.map((energy) => { const maximum = calculateCustomEnergyMaximum(energy); return <article className="custom-energy-card" key={energy.id}><div className="resource-line resource-line--energy"><Sparkles size={20} /><span>{energy.name}</span><div className="resource-bar resource-bar--arcane"><i style={{ width: `${Math.min(100, (energy.current / maximum) * 100)}%` }} /></div><label className="resource-value"><input aria-label={`${energy.name} atual`} type="number" min="0" max={maximum} value={energy.current} onChange={(event) => setCustomEnergyResource(energy.id, number(event.target.value), false)} onBlur={() => addLog(`Ajustou ${energy.name} diretamente para ${energy.current}.`, "health")} /><small>/{maximum}</small></label><button type="button" aria-label={`Diminuir ${energy.name} em 5`} onClick={() => setCustomEnergyResource(energy.id, energy.current - 5)}>−5</button><button type="button" aria-label={`Diminuir ${energy.name} em 1`} onClick={() => setCustomEnergyResource(energy.id, energy.current - 1)}><Minus size={14} /></button><button type="button" aria-label={`Aumentar ${energy.name} em 1`} onClick={() => setCustomEnergyResource(energy.id, energy.current + 1)}><Plus size={14} /></button><button type="button" aria-label={`Aumentar ${energy.name} em 5`} onClick={() => setCustomEnergyResource(energy.id, energy.current + 5)}>+5</button></div><details><summary>Configurar {energy.name}</summary><div className="custom-energy-card__fields"><label><span>Nome</span><input value={energy.name} onChange={(event) => updateCustomEnergy(energy.id, "name", event.target.value)} /></label><label><span>Máximo-base</span><input type="number" min="1" value={energy.maximum} onChange={(event) => updateCustomEnergy(energy.id, "maximum", number(event.target.value))} /></label><label><span>Bônus</span><input type="number" value={energy.bonus} onChange={(event) => updateCustomEnergy(energy.id, "bonus", number(event.target.value))} /></label><label><span>Máximo final</span><input type="number" value={maximum} readOnly /></label><label><span>Ícone</span><select value={energy.icon} onChange={(event) => updateCustomEnergy(energy.id, "icon", event.target.value)}>{CUSTOM_ENERGY_ICONS.map((icon) => <option key={icon}>{icon}</option>)}</select></label><label className="wide"><span>Descrição</span><input value={energy.description} onChange={(event) => updateCustomEnergy(energy.id, "description", event.target.value)} /></label></div><button type="button" className="row-delete" aria-label={`Remover energia ${energy.name}`} onClick={() => removeCustomEnergy(energy.id, energy.name)}><Trash2 size={14} /> Remover energia</button></details></article>; })}</div> : <small className="custom-energy-session__empty">Crie uma entrada da categoria Energia na Homebrew e use “Adicionar à ficha” para controlá-la como PF.</small>}</div>
              </div>
          ...
          <section id="combate" className={`codex-section ${activeSection === "combate" ? "is-active" : "is-hidden"}`}>
              ...
              <div className="combat-defenses">
                ...
                <div className="attack-list">
                  {sheet.attacks.map((attack) => <div className="attack-row" key={attack.id}>
                    ...
                    <label><span>Alcance</span><input value={attack.reach} onChange={(event) => updateAttack(attack.id, "reach", event.target.value)} /></label>
                    <label><span>Aparar</span><input value={attack.parry} onChange={(event) => updateAttack(attack.id, "parry", event.target.value)} /></label>
            ...
            <section className="strength-damage-panel" aria-label="Dano automático por força"><div><span className="eyebrow">DANO POR FORÇA</span><h3>Golpe e Balanço</h3><p>Referências atualizadas automaticamente pela ST {calculated.strengthDamage.st}. Os bônus somam-se à expressão final e ficam prontos para a rolagem.</p></div><div className="strength-damage-panel__actions"><button type="button" onClick={() => rollDamage(`Golpe · ST ${calculated.strengthDamage.st}`, `${calculated.strengthDamage.thrust} contusão`)}><span>GOLPE</span><b>{calculated.strengthDamage.thrust}</b><small>Rolar contusão</small></button><button type="button" onClick={() => rollDamage(`Balanço · ST ${calculated.strengthDamage.st}`, `${calculated.strengthDamage.swing} contusão`)}><span>BALANÇO</span><b>{calculated.strengthDamage.swing}</b><small>Rolar contusão</small></button></div><div className="strength-damage-panel__bonuses"><label><span>Bônus de Golpe</span><input type="number" value={sheet.secondary.thrustBonus || 0} onChange={(event) => updateSecondary("thrustBonus", number(event.target.value))} /></label><label><span>Bônus de Balanço</span><input type="number" value={sheet.secondary.swingBonus || 0} onChange={(event) => updateSecondary("swingBonus", number(event.target.value))} /></label></div></section>
            ...
            <div className="combat-powers"><div><span className="eyebrow">PODERES DE COMBATE</span><h3>Habilidades prontas para a cena</h3><p>Ative um poder para gastar FP, registrar o uso e executar uma rolagem 3d6.</p></div>{(sheet.powers || []).filter((power) => power.combatReady).length ? <div className="combat-powers__groups">{(["Ofensivo", "Defensivo", "Controle", "Utilidade"] as const).map((type) => {
              ...
              return <section className={`combat-powers__group combat-powers__group--${type.toLowerCase()}`} key={type}><header><span>{type}</span><b>{powers.length}</b></header><div className="combat-powers__list">{powers.map((power) => { const resource = getPowerResource(power); return <button type="button" key={power.id} onClick={() => usePower(power)} disabled={resource.current < resource.cost}><span><b>{power.name}</b><small>NH {getPowerNh(power)} · {formatEnergyCost(resource.name, resource.cost)}</small></span><i>{power.damage || power.effect || "Ativar"}</i><WandSparkles size={17} /></button>; })}</div></section>;
          ...
          <section id="inventario" className={`codex-section ${activeSection === "inventario" ? "is-active" : "is-hidden"}`}>
            <SectionHeader kicker="06 · CARGA" title="Equipamento" description="Controle o que está carregando e acompanhe o efeito sobre movimento e defesa em quilogramas." icon={Backpack} action={<div className="equipment-header-actions"><Button type="button" variant="outline" className="add-button" onClick={addBackpack}><Backpack size={15} /> Mochila</Button><Button type="button" variant="outline" className="add-button" onClick={() => setSheet((current) => ({ ...current, inventory: [...current.inventory, { id: makeId(), name: "Novo item", category: "Outros", quantity: 1, weight: 0, carried: true, equipped: false, hand: "—", modifier: "", description: "" }] }))}><PackagePlus size={15} /> Item</Button></div>} />
            <div className="inventory-layout"><div className="paper-card inventory-table"><div className="inventory-category-summary" aria-label="Resumo por categoria">{INVENTORY_CATEGORIES.map((category) => <span key={category}><b>{sheet.inventory.filter((item) => normalizeInventoryCategory(item.category) === category).length}</b>{category}</span>)}</div>{INVENTORY_CATEGORIES.map((category) => {
              const items = sheet.inventory.filter((item) => normalizeInventoryCategory(item.category) === category);
              if (!items.length) return null;
              return <section className="inventory-category-group" key={category}><header><div><span className="eyebrow">CATEGORIA</span><h3>{category}</h3></div><b>{items.length} {items.length === 1 ? "item" : "itens"}</b></header><div className="inventory-category-list">{items.map((item) => <article className="inventory-item-card" key={item.id}><div className="inventory-item-card__main"><label className="inventory-item-card__name"><span>Item</span><input value={item.name} aria-label={`Nome de ${item.name || "item"}`} onChange={(event) => updateItem(item.id, "name", event.target.value)} /></label><label><span>Categoria</span><select value={normalizeInventoryCategory(item.category)} aria-label={`Categoria de ${item.name || "item"}`} onChange={(event) => updateItem(item.id, "category", event.target.value)}>{INVENTORY_CATEGORIES.map((option) => <option key={option}>{option}</option>)}</select></label><label><span>Qtd.</span><input type="number" min="0" value={item.quantity} onChange={(event) => updateItem(item.id, "quantity", number(event.target.value))} /></label><label className="weight-input"><span>Peso un.</span><div><input type="number" min="0" step="0.1" value={item.weight} onChange={(event) => updateItem(item.id, "weight", number(event.target.value))} /><em>{UNIVERSAL_WEIGHT_UNIT}</em></div></label><label className="inventory-switch"><span>Carregado</span><span className="switch-label"><input type="checkbox" checked={item.carried} onChange={(event) => updateItem(item.id, "carried", event.target.checked)} /><i /></span></label><label className="inventory-switch"><span>Equipado</span><span className="switch-label"><input type="checkbox" checked={item.equipped} onChange={(event) => updateItem(item.id, "equipped", event.target.checked)} /><i /></span></label><button type="button" className="row-delete" aria-label={`Excluir ${item.name}`} onClick={() => removeItem(item.id, item.name)}><Trash2 size={14} /></button></div><div className="inventory-item-card__details"><label><span>Mão utilizada</span><select value={item.hand || "—"} aria-label={`Mão utilizada para ${item.name || "item"}`} onChange={(event) => updateItem(item.id, "hand", event.target.value)}>{INVENTORY_HANDS.map((option) => <option key={option}>{option}</option>)}</select></label><label><span>Bônus / penalidade</span><input value={item.modifier || ""} placeholder="Ex.: +1 dano, −1 DX" aria-label={`Bônus ou penalidade de ${item.name || "item"}`} onChange={(event) => updateItem(item.id, "modifier", event.target.value)} /></label><label className="wide"><span>Descrição</span><textarea className="item-description" value={item.description || ""} placeholder="Descrição do item" aria-label={`Descrição de ${item.name}`} onChange={(event) => updateItem(item.id, "description", event.target.value)} /></label></div></article>)}</div></section>;
            })}</div>
              <div className={`load-card load-card--${calculated.encumbrance >= 3 ? "danger" : calculated.encumbrance >= 1 ? "watch" : "safe"}`}><Weight size={23} /><span className="eyebrow">CARGA ATUAL</span><strong>{format(calculated.carriedWeight, 1)} <small>{UNIVERSAL_WEIGHT_UNIT}</small></strong><p>Carga básica: <b>{format(calculated.basicLift, 1)} {UNIVERSAL_WEIGHT_UNIT}</b></p><div className="load-scale">{[0, 1, 2, 3, 4].map((level) => <i key={level} className={calculated.encumbrance >= level ? "is-filled" : ""} />)}</div><div className="load-card__status"><span>{calculated.encName}</span><b>Movimento {calculated.move} · Esquiva {calculated.dodge}</b></div><small>{calculated.encumbrance >= 5 ? "A carga excede o limite de referência." : "Movimento e Esquiva já incluem a carga."}</small></div>
            </div>
          </section>
          ...
          <section id="aliados" className={`codex-section ${activeSection === "aliados" ? "is-active" : "is-hidden"}`}>
            ...
            <div className="ally-studio">
              ...
              {activeAlly ? <div className="ally-studio__detail"><header className="ally-sheet__header"><div><span className="eyebrow">MINI-FICHA DE ALIADO</span><h3>{activeAlly.name}</h3><p>{activeAlly.relation} · {activeAlly.type} · {activeAlly.race}</p></div><div className="ally-sheet__status"><span>Estado</span><AllyStatusSelect value={activeAlly.status} onValueChange={(status) => updateAlly(activeAlly.id, "status", status)} /><button type="button" aria-label={`Remover ${activeAlly.name}`} onClick={() => removeAlly(activeAlly.id, activeAlly.name)}><Trash2 size={15} /></button></div></header><div className="ally-sheet__body"><nav className="ally-tabs" aria-label="Abas da ficha do aliado">{allyTabs.map(({ id, label, icon: Icon }, index) => <button key={id} type="button" className={activeAllyTab === id ? "is-active" : ""} onClick={() => setActiveAllyTab(id)}><span>0{index + 1}</span><Icon size={15} />{label}</button>)}</nav><div className="ally-tab-panel">
                {activeAllyTab === "visao" && <div className="ally-tab-grid ally-tab-grid--overview">
                  <section className="ally-panel"><span className="eyebrow">IDENTIFICAÇÃO</span><div className="ally-field-grid">
                    <label className="wide"><span>Nome</span><input value={activeAlly.name} onChange={(event) => updateAlly(activeAlly.id, "name", event.target.value)} /></label>
                    <label><span>Relação</span><input value={activeAlly.relation} onChange={(event) => updateAlly(activeAlly.id, "relation", event.target.value)} /></label>
                    <label><span>Tipo</span><input value={activeAlly.type} onChange={(event) => updateAlly(activeAlly.id, "type", event.target.value)} /></label>
                    <label><span>Raça / espécie</span><input value={activeAlly.race} onChange={(event) => updateAlly(activeAlly.id, "race", event.target.value)} /></label>
                    <label><span>Aparência</span><input value={activeAlly.appearance} onChange={(event) => updateAlly(activeAlly.id, "appearance", event.target.value)} /></label>
                    <label className="wide"><span>Descrição</span><textarea value={activeAlly.description} onChange={(event) => updateAlly(activeAlly.id, "description", event.target.value)} /></label>
                    <label><span>Personalidade</span><textarea value={activeAlly.personality} onChange={(event) => updateAlly(activeAlly.id, "personality", event.target.value)} /></label>
                    <label><span>Motivação</span><textarea value={activeAlly.motivation} onChange={(event) => updateAlly(activeAlly.id, "motivation", event.target.value)} /></label>
                    <label className="wide"><span>Histórico</span><textarea value={activeAlly.history} onChange={(event) => updateAlly(activeAlly.id, "history", event.target.value)} /></label>
                  </div></section>
                  <section className="ally-panel ally-rules"><span className="eyebrow">VANTAGEM DE ALIADO</span><div className="ally-rules__cost"><span>Custo atual</span><strong>{calculateAllyCost(activeAlly)} <small>pts</small></strong><small>{activeAlly.powerPercent}% do personagem · aparece em {activeAlly.frequency} ou menos</small></div>
                    <label><span>Poder do aliado</span><select value={activeAlly.powerPercent} onChange={(event) => updateAllyData(activeAlly.id, { powerPercent: number(event.target.value) as 25 | 50 | 75 | 100 | 150 })}>{[25, 50, 75, 100, 150].map((power) => <option key={power} value={power}>{power}% · {allyPowerCosts[power as 25 | 50 | 75 | 100 | 150]} pts-base</option>)}</select></label>
                    <label><span>Frequência de aparecimento</span><select value={activeAlly.frequency} onChange={(event) => updateAllyData(activeAlly.id, { frequency: number(event.target.value) as 6 | 9 | 12 | 15 })}>{[6, 9, 12, 15].map((frequency) => <option key={frequency} value={frequency}>{frequency} ou menos · ×{allyFrequencyMultipliers[frequency as 6 | 9 | 12 | 15]}</option>)}</select></label>
                    <label className="ally-checkbox"><input type="checkbox" checked={activeAlly.isDependent} onChange={(event) => updateAlly(activeAlly.id, "isDependent", event.target.checked)} /><span>Também é um Dependente</span></label>
                    <label><span>Pontos atuais</span><input type="number" min="0" value={activeAlly.points} onChange={(event) => updateAlly(activeAlly.id, "points", number(event.target.value))} /></label>
                    <label><span>Observações</span><textarea value={activeAlly.notes} onChange={(event) => updateAlly(activeAlly.id, "notes", event.target.value)} /></label>
                  </section>
                </div>}
                {activeAllyTab === "atributos" && (() => { const secondary = activeAlly.secondary; const speed = (activeAlly.attributes.dx + activeAlly.attributes.ht) / 4 + (secondary.speedBonus || 0); const move = Math.max(1, Math.floor((secondary.moveBase || 5) + (secondary.moveBonus || 0))); const dodge = Math.max(1, Math.floor(speed) + 3 + (secondary.dodgeBonus || 0)); return <div className="ally-tab-grid ally-tab-grid--attributes"><section className="ally-panel"><span className="eyebrow">ATRIBUTOS PRIMÁRIOS</span><div className="ally-attribute-grid">{(["st", "dx", "iq", "ht"] as const).map((attribute) => <label key={attribute}><span>{attribute.toUpperCase()}</span><input type="number" min="1" value={activeAlly.attributes[attribute]} onChange={(event) => updateAllyAttribute(activeAlly.id, attribute, number(event.target.value))} /></label>)}</div></section><section className="ally-panel"><span className="eyebrow">RECURSOS E ATRIBUTOS SECUNDÁRIOS</span><div className="ally-field-grid"><label><span>PV atual</span><input type="number" min="0" value={activeAlly.hpCurrent} onChange={(event) => updateAlly(activeAlly.id, "hpCurrent", Math.max(0, Math.min(activeAlly.hpMax, number(event.target.value))))} /></label><label><span>PV máximo</span><input type="number" min="1" value={activeAlly.hpMax} onChange={(event) => updateAlly(activeAlly.id, "hpMax", Math.max(1, number(event.target.value)))} /></label><label><span>PF atual</span><input type="number" min="0" value={activeAlly.fpCurrent} onChange={(event) => updateAlly(activeAlly.id, "fpCurrent", Math.max(0, Math.min(activeAlly.fpMax, number(event.target.value))))} /></label><label><span>PF máximo</span><input type="number" min="1" value={activeAlly.fpMax} onChange={(event) => updateAlly(activeAlly.id, "fpMax", Math.max(1, number(event.target.value)))} /></label><label><span>Bônus de Vontade</span><input type="number" value={secondary.willBonus || 0} onChange={(event) => updateAllySecondary(activeAlly.id, "willBonus", number(event.target.value))} /></label><label><span>Bônus de Percepção</span><input type="number" value={secondary.perBonus || 0} onChange={(event) => updateAllySecondary(activeAlly.id, "perBonus", number(event.target.value))} /></label><label><span>Bônus de Velocidade</span><input type="number" step="0.25" value={secondary.speedBonus || 0} onChange={(event) => updateAllySecondary(activeAlly.id, "speedBonus", number(event.target.value))} /></label><label><span>Movimento-base</span><input type="number" min="1" value={secondary.moveBase || 5} onChange={(event) => updateAllySecondary(activeAlly.id, "moveBase", Math.max(1, number(event.target.value)))} /></label><label><span>Bônus de Movimento</span><input type="number" value={secondary.moveBonus || 0} onChange={(event) => updateAllySecondary(activeAlly.id, "moveBonus", number(event.target.value))} /></label><label><span>Bônus de Esquiva</span><input type="number" value={secondary.dodgeBonus || 0} onChange={(event) => updateAllySecondary(activeAlly.id, "dodgeBonus", number(event.target.value))} /></label></div><div className="ally-derived"><span>Vontade <b>{activeAlly.attributes.iq + (secondary.willBonus || 0)}</b></span><span>Percepção <b>{activeAlly.attributes.iq + (secondary.perBonus || 0)}</b></span><span>Velocidade <b>{speed.toFixed(2)}</b></span><span>Movimento <b>{move}</b></span><span>Esquiva <b>{dodge}</b></span></div></section></div>; })()}
          ...
          <section id="missoes" className={`codex-section ${activeSection === "missoes" ? "is-active" : "is-hidden"}`}>
            ...
            <div className="missions-list">{(sheet.missions || []).length ? (sheet.missions || []).map((mission, index) => <article className="mission-card" key={mission.id}><div className="mission-card__folio"><img src={MARK} alt="" /><span>MISSÃO</span><b>{String(index + 1).padStart(2, "0")}</b></div><div className="mission-card__main"><div className="mission-card__head"><input value={mission.title} aria-label="Título da missão" onChange={(event) => updateMission(mission.id, "title", event.target.value)} /><button type="button" className="row-delete" aria-label={`Excluir ${mission.title}`} onClick={() => removeMission(mission.id, mission.title)}><Trash2 size={14} /></button></div><div className="mission-fields"><label><span>Dificuldade</span><select value={mission.difficulty} onChange={(event) => updateMission(mission.id, "difficulty", event.target.value)}>{MISSION_DIFFICULTIES.map((difficulty) => <option key={difficulty}>{difficulty}</option>)}<option>Personalizada</option></select></label>{mission.difficulty === "Personalizada" && <label><span>Dificuldade da mesa</span><input value={mission.customDifficulty || ""} aria-label="Dificuldade personalizada da missão" placeholder="Ex.: Mortal, Caótica" onChange={(event) => updateMission(mission.id, "customDifficulty", event.target.value)} /></label>}<label><span>Estado</span><select value={mission.status} onChange={(event) => updateMission(mission.id, "status", event.target.value)}><option>Planejada</option><option>Em andamento</option><option>Concluída</option><option>Fracassada</option></select></label><label><span>Pontos ganhos</span><input type="number" min="0" value={mission.pointsReward} onChange={(event) => updateMission(mission.id, "pointsReward", Math.max(0, number(event.target.value)))} /></label><label><span>Dinheiro ganho</span><input type="number" min="0" value={mission.moneyReward} onChange={(event) => updateMission(mission.id, "moneyReward", Math.max(0, number(event.target.value)))} /></label><label><span>Moeda</span><input value={mission.currency} onChange={(event) => updateMission(mission.id, "currency", event.target.value)} /></label><label className="wide"><span>Relatório</span><textarea value={mission.notes} onChange={(event) => updateMission(mission.id, "notes", event.target.value)} /></label></div></div><div className="mission-rewards"><span>{mission.applied ? "RECOMPENSAS APLICADAS" : "RECOMPENSAS"}</span><strong>{mission.pointsReward} <small>pts</small></strong><b>{getMissionDifficultyLabel(mission)}</b><b>{mission.moneyReward} {mission.currency}</b><button type="button" onClick={() => applyMissionRewards(mission.id)} disabled={mission.applied}>{mission.applied ? "Aplicado" : "Concluir e aplicar"}</button></div></article>) : <button type="button" className="missions-empty" onClick={addMission}><span><ScrollText size={21} /></span><strong>Nenhuma missão registrada</strong><small>Crie um relatório para registrar a dificuldade, as recompensas e o resultado de uma aventura.</small><b><Plus size={14} /> Registrar primeira missão</b></button>}</div>
          </section>
