...
import { CUSTOM_ENERGY_ICONS, formatEnergyCost, normalizeCustomEnergy, type CustomEnergy, type CustomEnergyIcon } from "@shared/customEnergies";
...
export default function Home() {
  ...
  const calculated = useMemo(() => {
    ...
    const strengthDamage = calculateStrengthDamage(sheet.attributes.st);
  ...
  const updateCustomEnergy = (id: string, field: keyof CustomEnergy, value: string | number) => {
    setSheet((current) => ({ ...current, customEnergies: (current.customEnergies || []).map((energy) => {
      ...
      const candidate = { ...normalizeCustomEnergy(energy), [field]: field === "maximum" || field === "current" ? number(String(value)) : value } as CustomEnergy;
  ...
  const setCustomEnergyResource = (id: string, value: number, record = true) => {
    ...
    const next = clampResource(value, previousEnergy.maximum);
  ...
  const getPowerResource = (power: Power) => {
    ...
    return energy ? { id: energy.id, name: energy.name, current: energy.current, maximum: energy.maximum, cost } : { id: "pf", name: "PF", current: sheet.secondary.fpCurrent, maximum: calculated.fpMax, cost };
  };
      ...
      <main className="codex-main">
        ...
        <div className="codex-content">
          ...
          <section id="combate" className={`codex-section ${activeSection === "combate" ? "is-active" : "is-hidden"}`}>
            ...
            <section className="strength-damage-panel" aria-label="Dano automático por força"><div><span className="eyebrow">DANO POR FORÇA</span><h3>Golpe e Balanço</h3><p>Referências atualizadas automaticamente pela ST {calculated.strengthDamage.st}. Escolha o tipo de dano conforme o ataque.</p></div><div className="strength-damage-panel__actions"><button type="button" onClick={() => rollDamage(`Golpe · ST ${calculated.strengthDamage.st}`, `${calculated.strengthDamage.thrust} contusão`)}><span>GOLPE</span><b>{calculated.strengthDamage.thrust}</b><small>Rolar contusão</small></button><button type="button" onClick={() => rollDamage(`Balanço · ST ${calculated.strengthDamage.st}`, `${calculated.strengthDamage.swing} contusão`)}><span>BALANÇO</span><b>{calculated.strengthDamage.swing}</b><small>Rolar contusão</small></button></div></section>
          ...
          <section id="inventario" className={`codex-section ${activeSection === "inventario" ? "is-active" : "is-hidden"}`}>
            <SectionHeader kicker="06 · CARGA" title="Equipamento" description="Controle o que está carregando e acompanhe o efeito sobre movimento e defesa." icon={Backpack} action={<Button type="button" variant="outline" className="add-button" onClick={() => setSheet((current) => ({ ...current, inventory: [...current.inventory, { id: makeId(), name: "Novo item", category: "Outros", quantity: 1, weight: 0, carried: true, equipped: false, hand: "—", modifier: "", description: "" }] }))}><PackagePlus size={15} /> Item</Button>} />
