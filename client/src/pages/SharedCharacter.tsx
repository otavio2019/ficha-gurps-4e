...
export default function SharedCharacter() {
  ...
  const getPowerNh = (power: NonNullable<SharedSheet["powers"]>[number]) => {
    const linkedSkill = skills.find((skill) => skill.id === power.skillId);
    return calculateAttackNh(power.level, power.bonus || 0, linkedSkill ? { ...linkedSkill, relative: linkedSkill.relative || "" } : undefined, sheet.attributes);
  };
