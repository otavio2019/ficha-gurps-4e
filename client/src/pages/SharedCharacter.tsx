...
type SharedSheet = {
  ...
  powers?: Array<{ id: string; name: string; source: string; type: string; level: number; bonus?: number; fpCost: number; pointCost: number; range: string; duration?: string; area?: string; resistance?: string; prerequisites?: string; notes?: string; damage: string; effect: string; combatReady: boolean }>;
      ...
      <section className="shared-grid">
        ...
        <article className="shared-card shared-card--wide">
          ...
          <div className="shared-section-grid">
            ...
            <div><h4>Poderes</h4>{powers.length ? <div className="shared-list">{powers.map((power) => <div key={power.id}><b>{power.name}</b><span>{power.type} · NH {power.level + (power.bonus || 0)}</span><small>Fonte: {power.source || "não informada"} · Alcance: {power.range || "—"} · Duração: {power.duration || "—"}</small><small>{power.damage ? `Dano: ${power.damage}` : `Efeito: ${power.effect || "não informado"}`} · Área: {power.area || "alvo único"} · Resistência: {power.resistance || "nenhuma"} · {power.fpCost} PF · {power.pointCost} pts · {power.combatReady ? "pronto para combate" : "uso narrativo"}</small>{power.prerequisites && <small>Pré-requisitos: {power.prerequisites}</small>}{power.notes && <small>Observações: {power.notes}</small>}</div>)}</div> : <Empty>Nenhum poder cadastrado.</Empty>}</div>
          </div>
        </article>
        ...
        <article className="shared-card shared-card--wide">
          ...
          {inventory.length ? <div className="shared-item-grid">{inventory.map((item) => <div key={item.id}><b>{item.name}</b><span>{item.category || "Item"}</span><small>{item.quantity}× · {item.weight} kg {item.carried ? "· carregado" : ""}{item.equipped ? " · equipado" : ""}{item.description ? ` · ${item.description}` : ""}</small></div>)}</div> : <Empty>Nenhum equipamento cadastrado.</Empty>}
        </article>
